$(function(){
	...
	list: {
		...
		430010: [" 汽车维修/保养", "汽车维修/保养"],
		430008: ["二手车评估师", "Second-Hand Car Appraisers"],
		430011: ["汽车定损/车险理赔", "汽车定损/车险理赔"],
	...
	,i,j,m,n,k
	,html =''
	,hots = job.category.hotjobs
	,profs = job.category.jobs
	,jobList = job.list
	,relat = job.relations
	,tradeItem
	,$diaJ = $('.diaJ')
	,$selectC = $('#commonBox')
	,$selectP = $('#professBox')
	,$detail = $('#professDetail')
	,$taglist = $('#tagList')
	,$detailList = $('.detailList')
	,$headerlist = $('.headerTaglist')
	,$jobWrap = $('.jobWrap')
	,$inputVal = $('.textfield')
	,NUM = window.SELECT_Trade_NUM||5
//	,
	;
	...
	html += '	<h3>通用职能</h3>';
	for (i = 0,j = hots.length; i < j; i++) {
		tradeItem = hots[i];
		html += '<div class="trade" data-id="'+ tradeItem +'" data-name="' + jobList[tradeItem][0] + '">' + jobList[tradeItem][0] + '</div>';
	}
	html += '</div>';
	html += '<div class="profession">';
	html += '	<h3>专业职能</h3>';
	for (i = 0,j = profs.length; i < j; i++) {
		tradeItem = profs[i];
		html += '<div class="trade" data-id="'+ tradeItem +'" data-name="' + jobList[tradeItem][0] + '">' + jobList[tradeItem][0] + '</div>';
	}
	html += '</div>';
	...
	$('.common .trade').on('click',function() {
		var $that = $(this).addClass('active'),
			id = $that.data('id');
		$(this).addClass('active').siblings().removeClass('active');
		$selectC.html($.map(relat[id], function(d){
			return '<div class="tradeJob" data-id="' + d +'" data-name="'+ jobList[d][0] + '">' + jobList[d][0] + '</div>';
		}).join('') ).fadeIn();
	}),
	$d.on('click', '.control.cursor', function(event) {
		...
		switch(index) {
			case 0:
				$('#commonBox,#professBox,#professDetail').fadeOut('fast').siblings('.diaJ').show().find('.trade').removeClass('active');
				break;
			case 1:
				$detail.fadeOut('fast').siblings('#professBox').fadeIn('fast');
		}
		//
	})
			...
			/*$jobWrap.on({
					init: function(e, val){
						var choose = '';
						$jobWrap.find('.tradeJob.active,.selectDetail.active').removeClass('active');
						if( $.isArray( val ) ){//val的值是否是数组
							choose = $.map(val, function(d){
								return '<div class="headerTag">' + d + '<span class="del cursor">&times;</span></div>';
							});
							$headerlist.data('chooseNum', val.length);
							console.log(val)
							val = val.join('"],[data-name="');
							console.log(val)
						}
						else if( val ){
							$headerlist.data('chooseNum', 1);
							choose = '<div class="headerTag">'+ val + '<span class="del cursor">&times;</span></div>';
						}
						$headerlist.prevAll('.headerTag').remove().end().before( choose );
						$jobWrap.find('[data-name="'+ val +'"]').addClass('acitve');
					}
					, setValue: function(e, val, id){
						var $target = $d$jobWrap.data('inputTarget')
							;

						$target.prev().is(':hidden') && $target.prev().val(id);
						$target.val( val ).trigger('dialogChoose');
						$target.data('inputDialog').close();
					}
				})*/

	.on('click', '.profession .trade', function(event) {
		event.preventDefault();
		var $self = $(this)
		,id = $self.data('id')
		; 
		$('#jobControl').append('<div class="control cursor" data-id="'+ $self.data('id')+'">' + $self.text() + '</div>');
		$self.parents('.jobContent').hide();
		$selectP.html('<h3>专业职能</h3>' + $.map(relat[id], function(d){
			return '<div class="proTrade" data-id="' + d +'" data-name="'+ jobList[d][0] + '">' + jobList[d][0] + '</div>';
		}).join('') ).fadeIn('fast');	
	})
	.on('click','#professBox .proTrade',function(){
		var $self = $(this)
		,id = $self.data('id')
		,rel = relat[id]
		;
		$selectP.fadeOut('fast');
		$('#jobControl').append('<div class="control cursor" data-id="'+ $self.data('id')+'">' + $self.text() + '</div>');
		$taglist.html($.map(rel, function(d){
			return '<li class="selectTag" data-id="' + d +'" data-name="'+ jobList[d][0] + '">' + jobList[d][0] + '</li>';
		}).join('') );
		$taglist.append( $.map(hots, function(d){
			return '<li class="selectTag" data-id="'+ d +'" data-name="' + jobList[d][0] + '">'+ jobList[d][0] +'</li>';
		}).join('') );
		$detailList.html($.map(relat[rel[0]], function(d){
			return '<div class="selectDetail" data-id="' + d +'" data-name="'+ jobList[d][0] + '">' + jobList[d][0] + '</div>';
		}).join('') );

		console.log(relat[rel[0]])
		$taglist.find('.selectTag:eq(0)').trigger('click');
		$detail.fadeIn('fast');
		/*console.log($('#tagList'))*/
	})
	.on('click', '.selectTag', function(event) {
		event.preventDefault();
		var $self = $(this)
		,id = $self.data('id')
		;
		$self.addClass('active');//.siblings().removeClass('active');
		$detailList.html($.map(relat[id], function(d){
			return '<div class="selectDetail" data-id="' + d +'" data-name="'+ jobList[d][0] + '">' + jobList[d][0] + '</div>';
		}).join('') );
	})
	.on('click', '.tradeJob,.selectDetail', function(event) {
		event.preventDefault();
		var $self = $(this)
		,id = $self.data('id')
		,num = $headerlist.data('chooseNum')//向$headerlist中加数据
		//,numLimit = 0
		;
		//,numLimit = numLimit ? +numLimit : 0;
		num = num ? +num : 0;
		if(!$self.hasClass('active')){
			if( num < NUM ){
				$self.addClass('active');
				$headerlist.after('<div class="headerTag" title="'+ $self.data('name') +'" data-id="'+ $self.data('id') +'">' + $self.data('name') + '<span class="del cursor">&times;</span></div>');
				$headerlist.data('chooseNum', num +1);//数据chooseNum的数量值+1
			} else {
				alert(window.ALERT_TEXT_TRADE || '只能选择'+ NUM +'个');
			}
		} else {
			$self.removeClass('active');
			$headerlist.nextAll('.headerTag').filter(':contains("'+ $self.data('name') +'")').remove();
			$headerlist.data('chooseNum', num -1);
		}
	...
	.on('click', '.del', function(e) {
		...
		,num = $headerlist.data('chooseNum')
		;
		$self.find('.del').remove();
		$('.jobWrap').find('[data-name="' + $self.text() +'"]').removeClass('active');
		$self.remove();
		$headerlist.data('chooseNum', num -1);
	})
	.on('click', '#cancelBtn', function(event) {
		event.preventDefault();
		$jobWrap.hide();
	}).on('click', '#ensureBtn', function(event) {
		event.preventDefault();
		var id = []
		,val = $headerlist.nextAll('.headerTag').map(function() {
			var $self = $(this)
			;
			id.push($self.data('id') );
			return $self.find('span').remove().end().text();
		}).get().reverse().join();
		if( !val ){
			alert('meixuan');
		}else{
